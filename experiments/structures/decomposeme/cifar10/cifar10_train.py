...
class Train:
    ...
    def train(self):
        with self.graph.as_default():
            ...
            train_writer = tf.summary.FileWriter("summaries/train_dec_%d" % FLAGS.run_id, self.graph)
            test_writer = tf.summary.FileWriter("summaries/test_dec_%d" % FLAGS.run_id, self.graph)
