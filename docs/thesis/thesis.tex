...
\maketitle
\chapter*{Abstract}
There exists a gap between the computational cost of state of the art image processing models and the processing power of publicly available devices. This gap is reducing the applicability of these promising models. Trying to bridge this gap, first we investigate some methods to reduce the computational cost of a model. Second, we look for alternative operations to design state of the art models. Using these alternative operations, we train a model for CIFAR-10 classification task. Our model achieves similar results ($91.1\%$ top-1 accuracy) to ResNet-20 ($91.25\%$ top-1 accuracy) using a 581 KB model, requiring 3 times less operations. We observe that it is not possible to reduce the computational cost of such a model using the techniques we have investigated previously.
%\chapter*{Dedication}
%To mum and dad
